...
interface. The task state is stored in the
`.gemini/sessions/tracker/<session-id>` directory, allowing the agent to manage
its plan for the current session.
...
- **Persistence:** Task state is saved automatically to the
  `.gemini/sessions/tracker/<session-id>` directory. Task states are
